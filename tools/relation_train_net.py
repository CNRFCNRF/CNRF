...
def train(cfg, local_rank, distributed, logger):
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        if cfg.SOLVER.TO_VAL and iteration % cfg.SOLVER.VAL_PERIOD == 0:
            ...
            val_result = run_val(cfg, model, val_data_loaders, distributed, logger, test=False)
...
def run_val(cfg, model, val_data_loaders, distributed, logger, test):
    if distributed:
        model = model.module
...
def run_test(cfg, model, distributed, logger, test):
    if distributed:
        model = model.module
    torch.cuda.empty_cache()
    iou_types = ("bbox",)
