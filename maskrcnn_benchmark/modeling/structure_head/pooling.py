...
class Compute_Source(nn.Module):
    def __int__(self, source_input, source_output):
        self.compute_source = nn.Sequential(
            nn.Linear(source_input, source_output),
            nn.ReLU(),
        )
    def forward(self, im_pooled, sg_pooled):
        source = im_pooled * sg_pooled
        source = self.compute_source(source)
        return source
