...
def get_predicted_sg(detections, num_obj_classes, mode, noise_var):
    ...
    -----------
        ...
        该函数将场景中的检测转换为grpah结构
参数:
-----------
检测:一个元组(relation_logits, object_logits, rel_pair_idxs，提案)
    '''
    offset = 0
    pair_list = []
    batch_list = []
...
images:批量输入图像
检测:一个元组(relation_logits, object_logits, rel_pair_idxs，提案)
base_model:关系预测模型(用于提取特征)
num_obj_classes:数据集中对象类的数量(用于ocnvertin转换为一个热编码)
返回:
----------
im_graph:与图像相对应的图形
scene_graph:与场景图相对应的图
    '''
