...
class TransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.dim = 512
        self.generate_subnodes_feats = GenerateSubnodes(self.dim)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features,
        ...
        cross_head_features, cross_tail_features = self.generate_subnodes_feats(proposals, cross_head_features,
                                                                                cross_tail_features,
        ...
        cross_head_features = cross_head_features.split(num_rels, dim=0)
        cross_tail_features = cross_tail_features.split(num_rels, dim=0)
        prod_reps = []
        pair_preds = []
        cross_preds = []
        global_preds = []
        for pair_idx, head_rep, tail_rep, obj_pred, head, tail, num in zip(
                rel_pair_idxs, head_reps, tail_reps, obj_preds, cross_head_features, cross_tail_features, num_objs):
            global_head = head_rep[pair_idx[:, 0]]
            global_tail = tail_rep[pair_idx[:, 1]]

            prod_reps.append(
                torch.cat((global_head * (head + 1), global_tail * (tail + 1)), dim=-1))
            global_rep = torch.cat((global_head, global_tail), dim=-1)
            cross_rep = torch.cat((head, tail), dim=-1)
            global_preds.append(global_rep)
            cross_preds.append(cross_rep)
        ...
        global_rel_dists = self.rel_compress(global_preds * union_features)

        cross_rel_dists = self.rel_compress(cross_preds)
...
class VCTreePredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        # self.node_dim = 512
        # self.gnn = GNNLayer(self.node_dim)
...
class CausalAnalysisPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features,
        ...
        return obj_dist_list, rel_dist_list, add_losses, cross_rel_dists, global_rel_dists
