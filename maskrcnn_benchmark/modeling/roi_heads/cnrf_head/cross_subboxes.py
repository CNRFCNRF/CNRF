...
def generate_cross_subboxes(proposals, rel_pair_idxs):
    ...
    longitudinal_matrix = longitudinal_matrix1 + longitudinal_matrix2
    ...
    longitudinal_boxes_head = longitudinal_matrix * torch.cat((torch.min(inter_x1, center_h_x), torch.min(inter_y1, h_y_1),
                                                          torch.max(inter_x2, center_h_x), torch.max(inter_y2, h_y_2)),
                                                         dim=-1)
    longitudinal_boxes_tail = longitudinal_matrix * torch.cat((torch.min(inter_x1, center_t_x), torch.min(inter_y1, t_y_1),
    ...
    interaction_h_boxes = horizontal_boxes_head + longitudinal_boxes_head + \
                          + subset_up_boxes_head + subset_down_boxes_head
    interaction_t_boxes = horizontal_boxes_tail + longitudinal_boxes_tail +  \
    ...
    non_interaction_h_boxes, non_interaction_t_boxes = generate_non_interaction_boxes(condition_1, condition_2, zeros,
                                                                                      interaction_matrix,
                                                                                      h_x_1, h_y_1, h_x_2, h_y_2, t_x_1, t_y_1,
                                                                                      t_x_2, t_y_2,
                                                                                      center_h_x,
    ...
    return cross_head_boxes, cross_tail_boxes


def generate_non_interaction_boxes(condition_1, condition_2, zeros, youjiaoji_matrix,
                                   x1, y1, x2, y2, x3, y3, x4, y4, center_head_x, center_head_y, center_tail_x, center_tail_y):
    ...
    no_interaction_head = no_interaction_matrix * torch.cat((torch.min(x4, center_head_x), torch.min(y4, center_head_y),
                                                     torch.max(x3, center_head_x), torch.max(y3, center_head_y)),
                                                    dim=-1)
    no_interaction_tail = no_interaction_matrix * torch.cat((torch.min(x2, center_tail_x), torch.min(y2, center_tail_y),
                                                     torch.max(x1, center_tail_x), torch.max(y1, center_tail_y)),
                                                    dim=-1)
