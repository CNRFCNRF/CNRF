...
class GenerateReassignmentLabels(nn.Module):
    def __init__(self, predictor):
        super(GenerateReassignmentLabels, self).__init__()
        self.predictor = predictor

    def forward(self, global_rel_dists, rel_labels):
        ...
        if self.predictor != "CausalAnalysisPredictor":
            ...
            reassignment_labels = torch.cat(
                (reassignment_score_1, reassignment_score_2, reassignment_score_3, reassignment_score_4,
                 reassignment_score_5),
                dim=-1)
